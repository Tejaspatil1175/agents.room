  ...
  return (
    <div className="w-full">
      <div className="flex flex-col items-center sm:items-start mb-8">
        ...
        <p className="text-sm text-zinc-500">Start building agents in 30 seconds. Free forever.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className={`w-full h-11 px-4 rounded-xl border ${errors.name ? 'border-red-300 focus:ring-red-500' : 'border-zinc-200 focus:ring-zinc-900'} bg-white text-[14px] text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-150`}
        ...
        <div>
          ...
          <div className="mt-2 text-xs flex items-center justify-between">
              ...
              {[1, 2, 3, 4].map((index) => (
                <div 
                  ...
                  className={`h-1 w-full rounded-full transition-colors duration-300 ${
                    formData.password.length > 0 && passwordStrength >= index 
                      ? (strengthColors as any)[passwordStrength] || "bg-emerald-500" 
                      : "bg-zinc-200"
                  }`}
                ></div>
              ))}
            ...
            <span className={`min-w-[40px] text-right font-medium ${
              formData.password.length === 0 ? "text-zinc-400" :
              passwordStrength <= 1 ? "text-red-500" :
              passwordStrength === 2 ? "text-amber-500" : "text-emerald-500"
            }`}>
              {formData.password.length === 0 ? "Weak" : strengthLabels[passwordStrength]}
            </span>
          </div>
        </div>
        ...
        <div className="py-2">
           <p className="text-xs text-zinc-500 text-center">
            ...
            <Link href="#" className="text-zinc-700 underline underline-offset-2">Terms</Link>
            {" "}and{" "}
            <Link href="#" className="text-zinc-700 underline underline-offset-2">Privacy Policy</Link>
           </p>
        </div>
