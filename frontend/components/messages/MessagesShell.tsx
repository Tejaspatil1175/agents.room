...
export default function MessagesShell() {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden bg-[#0a0a0a]">
      {/* Sidebar */}
      <div className="w-80 border-r border-white/[0.06] flex flex-col bg-[#0d0d0d]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {agents.length === 0 ? (
            <div className="p-6 text-center text-zinc-600 text-[13px]">
              No agents found. Create one to see messages.
            </div>
          ) : (
            agents.map((agent) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 border-b border-white/[0.04] transition-colors hover:bg-white/[0.02] ${
                    selectedAgentId === agent._id ? "bg-white/[0.06] border-r-2 border-r-emerald-500" : ""
                  }`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-[14px] font-medium text-zinc-100 truncate">{agent.name}</span>
                    {lastMsg && (
                  ...
                  <div className="text-[12px] text-zinc-500 truncate h-4">
                    {lastMsg ? (lastMsg.status === 'success' ? lastMsg.output : lastMsg.error_message) : `New ${agent.type} agent`}
                  </div>
                </button>
              );
            })
      ...
      <div className="flex-1 flex flex-col bg-[#0a0a0a]">
        {selectedAgent ? (
          <>
            <div className="h-16 px-8 border-b border-white/[0.06] flex items-center justify-between bg-[#0d0d0d]/50 backdrop-blur-sm sticky top-0 z-10">
            ...
            >
              {filteredMessages.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                  <div className="w-16 h-16 rounded-full bg-white/[0.02] border border-white/[0.04] flex items-center justify-center mb-4">
                  ...
                  <h4 className="text-[15px] font-medium text-zinc-300 mb-1">No messages yet</h4>
                  <p className="text-[13px] text-zinc-500">When your agent runs according to its schedule, you'll see the results here.</p>
                </div>
              ) : (
                filteredMessages.map((msg) => (
                  <div key={msg._id} className="flex flex-col gap-2 max-w-[85%]">
                    ...
                    <span className="text-[10px] text-zinc-600 ml-2">
                       {new Date(msg.ran_at).toLocaleString()}
                    </span>
                  </div>
                ))
              )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <h3 className="text-[17px] font-semibold text-zinc-200 mb-2">Your Conversations</h3>
            <p className="text-[14px] text-zinc-500 max-w-xs">Select an agent from the sidebar to view its latest output and run history.</p>
          </div>
        )}
      </div>
